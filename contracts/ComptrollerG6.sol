...
contract ComptrollerG6 is ComptrollerV6Storage, ComptrollerWithTermLoansInterface, ComptrollerErrorReporter, Exponential {
    ...
    event MarketEntered(CTokenWithTermLoans cToken, address account);

    /// @notice Emitted when an account exits a market
    event MarketExited(CTokenWithTermLoans cToken, address account);

    /// @notice Emitted when close factor is changed by admin
    event NewCloseFactor(uint oldCloseFactorMantissa, uint newCloseFactorMantissa);

    /// @notice Emitted when a collateral factor is changed by admin
    event NewCollateralFactor(CTokenWithTermLoans cToken, uint oldCollateralFactorMantissa, uint newCollateralFactorMantissa);
    ...
    event NewPriceOracle(PriceOracleWithTermLoans oldPriceOracle, PriceOracleWithTermLoans newPriceOracle);
    ...
    event ActionPaused(CTokenWithTermLoans cToken, string action, bool pauseState);

    /// @notice Emitted when market comped status is changed
    event MarketComped(CTokenWithTermLoans cToken, bool isComped);

    /// @notice Emitted when COMP rate is changed
    event NewCompRate(uint oldCompRate, uint newCompRate);

    /// @notice Emitted when a new COMP speed is calculated for a market
    event CompSpeedUpdated(CTokenWithTermLoans indexed cToken, uint newSpeed);

    /// @notice Emitted when COMP is distributed to a supplier
    event DistributedSupplierComp(CTokenWithTermLoans indexed cToken, address indexed supplier, uint compDelta, uint compSupplyIndex);
     ...
     */
    function getAssetsIn(address account) external view returns (CTokenWithTermLoans[] memory) {
        CTokenWithTermLoans[] memory assetsIn = accountAssets[account];
     ...
     */
    function checkMembership(address account, CTokenWithTermLoans cToken) external view returns (bool) {
    ...
    function enterMarkets(address[] memory cTokens) public returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            CTokenWithTermLoans cToken = CTokenWithTermLoans(cTokens[i]);
     ...
     */
    function addToMarketInternal(CTokenWithTermLoans cToken, address borrower) internal returns (Error) {
    ...
    function exitMarket(address cTokenAddress) external returns (uint) {
        CTokenWithTermLoans cToken = CTokenWithTermLoans(cTokenAddress);
        ...
        CTokenWithTermLoans[] memory userAssetList = accountAssets[msg.sender];
        ...
        CTokenWithTermLoans[] storage storedList = accountAssets[msg.sender];
    ...
    function redeemAllowedInternal(address cToken, address redeemer, uint redeemTokens) internal view returns (uint) {
        ...
        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal(redeemer, CTokenWithTermLoans(cToken), redeemTokens, 0);
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount, uint loanIndex) external returns (uint) {
        ...
        if (!markets[cToken].accountMembership[borrower]) {
            ...
            Error err = addToMarketInternal(CTokenWithTermLoans(msg.sender), borrower);
            if (err != Error.NO_ERROR) {
                return uint(err);
            }
        ...
        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, CTokenWithTermLoans(cToken), 0, borrowAmount);
        if (err != Error.NO_ERROR) {
            return uint(err);
        }
        if (shortfall > 0) {
        ...
        // Keep the flywheel moving
        Exp memory borrowIndex = Exp({mantissa: CTokenWithTermLoans(cToken).borrowIndex()});
    ...
    function liquidateBorrowAllowed(
        ...
        uint borrowBalance = CTokenWithTermLoans(cTokenBorrowed).borrowBalanceStored(borrower, loanIndex);
    ...
    function seizeAllowed(
        ...
        if (CTokenWithTermLoans(cTokenCollateral).comptroller() != CTokenWithTermLoans(cTokenBorrowed).comptroller()) {
    ...
    function getAccountLiquidity(address account) public view returns (uint, uint, uint) {
        (Error err, uint liquidity, uint shortfall) = getHypotheticalAccountLiquidityInternal(account, CTokenWithTermLoans(0), 0, 0);
    ...
    function getAccountLiquidityInternal(address account) internal view returns (Error, uint, uint) {
        return getHypotheticalAccountLiquidityInternal(account, CTokenWithTermLoans(0), 0, 0);
    }
    ...
    function getHypotheticalAccountLiquidity(
        ...
        (Error err, uint liquidity, uint shortfall) = getHypotheticalAccountLiquidityInternal(account, CTokenWithTermLoans(cTokenModify), redeemTokens, borrowAmount);
        return (uint(err), liquidity, shortfall);
    }
    ...
    function getHypotheticalAccountLiquidityInternal(
        address account,
        CTokenWithTermLoans cTokenModify,
        uint redeemTokens,
        ...
        CTokenWithTermLoans[] memory assets = accountAssets[account];
        for (uint i = 0; i < assets.length; i++) {
            CTokenWithTermLoans asset = assets[i];
    ...
    function liquidateCalculateSeizeTokens(address cTokenBorrowed, address cTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
        ...
        uint priceBorrowedMantissa = oracle.getUnderlyingPrice(CTokenWithTermLoans(cTokenBorrowed));
        uint priceCollateralMantissa = oracle.getUnderlyingPrice(CTokenWithTermLoans(cTokenCollateral));
         ...
         */
        uint exchangeRateMantissa = CTokenWithTermLoans(cTokenCollateral).exchangeRateStored(); // Note: reverts on error
        uint seizeTokens;
    ...
    /**
      ...
      */
    function _setPriceOracle(PriceOracleWithTermLoans newOracle) public returns (uint) {
        ...
        PriceOracleWithTermLoans oldOracle = oracle;
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure. (See ErrorReporter for details)
      */
    function _setCollateralFactor(CTokenWithTermLoans cToken, uint newCollateralFactorMantissa) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
    ...
    /**
      ...
      */
    function _supportMarket(CTokenWithTermLoans cToken) external returns (uint) {
        if (msg.sender != admin) {
    ...
    function _setMintPaused(CTokenWithTermLoans cToken, bool state) public returns (bool) {
    ...
    function _setBorrowPaused(CTokenWithTermLoans cToken, bool state) public returns (bool) {
        require(markets[address(cToken)].isListed, "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == admin, "only pause guardian and admin can pause");
        require(msg.sender == admin || state == true, "only admin can unpause");
    ...
    function refreshCompSpeedsInternal() internal {
        CTokenWithTermLoans[] memory allMarkets_ = allMarkets;

        for (uint i = 0; i < allMarkets_.length; i++) {
            CTokenWithTermLoans cToken = allMarkets_[i];
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            CTokenWithTermLoans cToken = allMarkets[i];
    ...
    function updateCompSupplyIndex(address cToken) internal {
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = CTokenWithTermLoans(cToken).totalSupply();
    ...
    function updateCompBorrowIndex(address cToken, Exp memory marketBorrowIndex) internal {
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(CTokenWithTermLoans(cToken).totalBorrows(), marketBorrowIndex);
    ...
    function distributeSupplierComp(address cToken, address supplier, bool distributeAll) internal {
        ...
        uint supplierTokens = CTokenWithTermLoans(cToken).balanceOf(supplier);
        uint supplierDelta = mul_(supplierTokens, deltaIndex);
        uint supplierAccrued = add_(compAccrued[supplier], supplierDelta);
        compAccrued[supplier] = transferComp(supplier, supplierAccrued, distributeAll ? 0 : compClaimThreshold);
        emit DistributedSupplierComp(CTokenWithTermLoans(cToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
    ...
    function distributeBorrowerComp(address cToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint borrowerAmount = div_(CTokenWithTermLoans(cToken).allBorrowBalanceStored(borrower), marketBorrowIndex);
            uint borrowerDelta = mul_(borrowerAmount, deltaIndex);
            uint borrowerAccrued = add_(compAccrued[borrower], borrowerDelta);
            compAccrued[borrower] = transferComp(borrower, borrowerAccrued, distributeAll ? 0 : compClaimThreshold);
            emit DistributedBorrowerComp(CTokenWithTermLoans(cToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
     ...
     */
    function claimComp(address holder, CTokenWithTermLoans[] memory cTokens) public {
     ...
     */
    function claimComp(address[] memory holders, CTokenWithTermLoans[] memory cTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < cTokens.length; i++) {
            CTokenWithTermLoans cToken = cTokens[i];
    ...
    function _addCompMarketInternal(address cToken) internal {
        ...
        emit MarketComped(CTokenWithTermLoans(cToken), true);
    ...
    function _dropCompMarket(address cToken) public {
        ...
        emit MarketComped(CTokenWithTermLoans(cToken), false);
     ...
     */
    function getAllMarkets() public view returns (CTokenWithTermLoans[] memory) {
        return allMarkets;
    }
